...
const applicationsController = {
  ...
  postNewApplication: async (req, res) => {
    try {
      const { adopter_id, pet_id, shelter_id, status, submission_date, notes } =
        req.body;

      // Validate required fields
      if (!adopter_id || !pet_id || !shelter_id) {
        return res.status(400).json({
          message: "Missing required ID fields in request body",
        });
      }

      const newApp = await applicationsModel
        .getCollection("applications")
        .insertOne({
      ...
      if (
        ...
        error.message.includes("Argument passed in must be a string")
      ) {
        return res.status(400).json({
          message: "Invalid 24-character hexadecimal ID format in payload",
        });
      }

      return res.status(500).json({
        message: "Internal Server Error posting new application",
      });
    }
  },
  ...
  editApplication: async (req, res) => {
    try {
      ...
      if (submission_date)
        updateFields.submission_date = new Date(submission_date);
      if (notes) updateFields.notes = notes;

      const editApp = await applicationsModel
        .getCollection("applications")
        .updateOne({ _id: new _id(req.params.id) }, { $set: updateFields });
  ...
  deleteApplication: async (req, res) => {
    try {
      const deleteApp = await applicationsModel
        .getCollection("applications")
        .deleteOne({ _id: new _id(req.params.id) });
